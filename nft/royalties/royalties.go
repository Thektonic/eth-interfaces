...
func NewERC721RoyaltiesInteractions(baseIERC721 nft.INFT, signatures []IERC721RoyaltiesSignature) (*IERC721RoyaltiesInteractions, error) {
	ierc721Royalties, err := ERC721Complete.NewERC721Complete(baseIERC721.GetAddress(), baseIERC721.GetBaseInteractions().Client)
	...
	callError := func(field string, err error) *base.CallError {
		return baseIERC721.GetBaseInteractions().WrapCallError(ERC721Complete.ERC721CompleteABI, "nft.RoyaltyInfo()", err)
	}

	return &IERC721RoyaltiesInteractions{
		&base.Interactions[*ERC721Complete.ERC721CompleteSession]{
...
func (e *IERC721RoyaltiesInteractions) RoyaltiesInfos(tokenID *big.Int, salePrice *big.Int) (RoyaltyInfos, error) {
	rInfos, err := e.Session.RoyaltyInfo(tokenID, salePrice)
	if err != nil {
